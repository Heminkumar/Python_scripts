...
def report(SN,Headers):
    filename = "Summary_"
    with open(filename+ time.strftime("%d_%B_%Y-%H_%M_%S")+".csv", "w") as outfile:
        writer = csv.writer(outfile)
        #writer1 = csv.DictWriter(outfile, fieldnames=Headers)
        #writer.writeheader()
        writer.writerow(SN.keys())
        writer.writerows(zip(*SN.values()))

# def header_write(Headers):
#     file = 'Summary' + time.strftime("%Y-%m-%d-%H:%M:%S") +'.csv' 
